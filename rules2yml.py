                            ...
                            if matchComment is None and len(saveStatement) == 0:
                                if numMessage > 0 or isEnum == True:
                                    if statement.find(";") != -1:
                                        ...
                                        schema_file.write(
                                            (2 * numMessage) * " " + f"{field}: any(list(include('rules', required=False)), null(), required=False)\n"
                                        )
                                        prevMainField = False
                schema_file.write(
                    "---\n"
                    "rules:\n"
                    "  is_greater_than: int(required=False)\n"
                    "  is_greater_than_or_equal_to: int(required=False)\n"
                    "  is_less_than_or_equal_to: int(required=False)\n"
                    "  is_less_than: int(required=False)\n"
                    "  is_equal_to: any(int(), bool(), required=False)\n"
                    "  is_different_to: int(required=False)\n"
