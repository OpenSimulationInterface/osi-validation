...
rules_dict = {'in_range': r'\b(in_range)\b: \[\d+(\.\d+)?, \d+(\.\d+)?\]',
              ...
              'is_greater_than_or_equal_to': r'\b(is_greater_than_or_equal_to)\b',
              'is_less_than_or_equal_to': r'\b(is_less_than_or_equal_to)\b',
              'is_less_than': r'\b(is_less_than)\b: \d+(\.\d+)?',
              'is_equal_to': r'\b(is_equal_to)\b',
              ...
              'first_element': r'\b(first_element)\b',
              'last_element': r'\b(last_element)\b',
...
for file in glob("open-simulation-interface/*.proto*"):    
    ...
    if os.path.exists(f"{rule_dir}/{filename}.yml"):
        continue

    with open(f"{rule_dir}/{filename}.yml", "a") as yml_file:
                    ...
                    if isEnum is False:
                        ...
                        if matchMessage is not None:
                            ...
                            if matchName is not None:
                                ...
                                yml_file.write(2*(numMessage-1)*" "+f"{matchNameConv.group(0)}:\n")
