...
rules_dict = {
    "in_range": r"\b(in_range)\b: \[\d+(\.\d+)?, \d+(\.\d+)?\]",
    ...
    "is_equal_to": r"\b(is_equal_to)\b: \d+(\.\d+)?",
    "is_different_to": r"\b(is_different_to)\b: \d+(\.\d+)?",
    "is_globally_unique": r"\b(is_globally_unique)\b",
    "refers_to": r"\b(refers_to)\b",
        ...
        for line in fin:
            ...
            if sys.version_info >= (3, 0):
                if (
                    ...
                    print(
                        file + " in line " + str(i) + ": a none ASCII char is present"
                    )
                    state = 1
            else:
                if line != unicodedata.normalize(
                    "NFKD", unicode(line, "ISO-8859-1")
                ).encode("ASCII", "ignore"):
                    print(
                        file + " in line " + str(i) + ": a none ASCII char is present"
                    )
                ...
                if matchEnum is not None:
                    ...
                    if matchName is not None:
                        ...
                        if matchNameConv is None:
                            ...
                            state = 1
                        enumName = (
                            convert(endOfLine[matchName.start() : matchName.end()])
                            + "_"
                        )
                        ...
                        if noMessage > 0:
                            ...
                            if matchName is not None:
                                ...
                                if matchName is not None:
                                    checkType = (
                                        " "
                                        + type[matchName.start() : matchName.end() - 1]
                                        + " "
                                    )
                                ...
                                if (
                                    ...
                                    and re.search(r"\brepeated\b", type) is None
                                ):
                ...
                if matchComment is not None:
                    ...
                    if htmlblock is True:
                        ...
                        if matchEndHTMLOnly is not None:
                            htmlFreeComment = (
                                htmlFreeComment + htmlComment[matchEndHTMLOnly.end() :]
                            )
                    ...
                    if matchComment is not None:
                        ...
                        if noMessage > 0 or isEnum == True:
                            if statement.find(";") != -1:
                                ...
                                if (
                                    hasRule
                                    and lineruleCount != foundruleCount
                                    and endRule
                                    and lineruleCount - foundruleCount - 1 > 0
                                ):
                                    print(
                                        file
                                        + " in line "
                                        + str(i)
                                        + ": "
                                        + str(lineruleCount - foundruleCount - 1)
                                        + " defined rule(s) does not exists for: '"
                                        + statement
                                        + "'"
                                    )
                                    state = 1
                                ...
                                if (
                                    ...
                                    print(
                                        ...
                                        + " in line "
                                        + str(i)
                                        + ": endrules statement does not exists for: '"
                                        + statement
                                        + "'"
                                    )
                                    state = 1
